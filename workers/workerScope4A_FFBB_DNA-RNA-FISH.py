master_analysis_folder = r'C:\Scripts\NMERFISH_Jenny'

### Did you compute PSF and median flat field images?
psf_file = r'C:\Scripts\NMERFISH_Jenny\psfs\psf_750_Scope4_FFBB.npy'
flat_field_tag = r'C:\Scripts\NMERFISH_Jenny\flat_field\Scope4FFBB_'#med_col_rawXXX.npz

master_data_folder = [r'\\192.168.0.3\bbfishdc10\TK_FFBBL1_SampleA_1_27_2023']
save_folder =r'\\192.168.0.3\bbfishdc10\TK_FFBBL1_SampleA_1_27_2023__JennyAnalysis'
...
def get_iH(fld): 
    """
    Given a folder <fld> of type \\192.168.0.3\bbfishdc10\TK_FFBBL1_SampleA_1_27_2023\H2Q4_5_6 this extracts the hybe index (i.e. 2 in this case)
    """
    try:
        return int(os.path.basename(fld).replace('R','_R').replace('Q','_Q').split('_')[0][1:])
    except:
        return np.inf
...
def get_files(ifov):
    ...
    for master_folder in master_data_folder:
        all_flds += glob.glob(master_folder+r'\H*R*')
