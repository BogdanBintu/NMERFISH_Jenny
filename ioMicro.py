...
def get_local_max_tile(im_,th=2500,s_ = 500,pad=50,psf=None,plt_val=None,snorm=30,gpu=False,deconv={'method':'wiener','beta':0.001},
    ...
    for ix,iy in ixys:#tqdm(ixys):
        ...
        #print(time.time()-t)
        Xh = get_local_maxfast_tensor(out_im2,th,im_raw=imsm,dic_psf=None,delta=delta,delta_fit=delta_fit,sigmaZ=sigmaZ,sigmaXY=sigmaXY,gpu=False)
...
def load_segmentation(dec):
    fls_ =  glob.glob(dec.save_folder+os.sep+'Segmentation'+os.sep+dec.fov.split('.')[0]+'*'+dec.set_+'*.npz')
    if len(fls_)==0:
        return 0
    dec.fl_dapi =fls_[0]
...
def check_image(dec,tag = '_MER'):
    result = np.load(dec.drift_fl,allow_pickle=True)
    drifts,flds,fov_ = result[:3]
    fl_ref = result[-1]
    ...
    if True:
        ...
        from dask import array as da
        im  = da.concatenate([da.roll(read_im(fld_+os.sep+fov_),np.round(drft[0]).astype(int),axis=[1,2,3])[np.newaxis]for fld_,drft in zip(flds,drifts)
...
def plot_gene_scdata(scdata2,gene='SOX9',nmax=20,sz_min=5,sz_max=30,transpose=1,flipx=1,flipy=1,tag='X_spatial'):
    ...
    plt.yticks([])
    return fig
def plot_cluster_scdata(scdata,cmap,clusters=[1,2],transpose=1,flipx=1,flipy=1,sbig=30,small=5,tag_cluster='leiden'):
    import matplotlib.pyplot as plt
    fig = plt.figure(figsize=(10, 10), facecolor="black")

    from matplotlib import pylab as plt
    x,y = (scdata.obsm['X_spatial']*[-flipx,-flipy])[:,::-transpose].T
    clusters_all = scdata.obs[tag_cluster]
    clusters_unique = list(np.unique(clusters_all).astype(str))
    #np.unique(scdata.obs["leiden"].astype(np.int))[::-1]
    plt.scatter(x, y, c='gray', s=small, marker='.')
    for cluster in clusters:
        cluster_ = str(cluster)
        inds = clusters_all == cluster_
        x_ = x[inds]
        y_ = y[inds]
        icl = clusters_unique.index(cluster_)
        col = cmap[icl % len(cmap)]
        plt.scatter(x_, y_, c=col, s=sbig, marker='.',label = cluster_)
...
def plot_gene_scdata(scdata2,gene='SOX9',nmax=20,sz_min=5,sz_max=30,transpose=1,flipx=1,flipy=1,tag='X_spatial',recompute_raw=False):
    ...
    plt.yticks([])
    return fig
def plot_gene_scdata_napari(scdata2,gene='SOX9',nmax=20,sz_min=5,sz_max=30,transpose=1,flipx=1,flipy=1,tag='X_spatial',viewer=None):
    Xcells = scdata2.obsm[tag][:,::transpose]*[flipx,flipy]
    ign = list(scdata2.var.index).index(gene)
    #scdata2.obsm['X_umap']
    if 'X_raw' not in scdata2.obsm:
...
class get_dapi_features:
    ...
    def load_im(self):
        ...
        imD = full_deconv(im,psf=self.psf,gpu=self.gpu,parameters={'method': 'wiener', 'beta': 0.001},force=True)
